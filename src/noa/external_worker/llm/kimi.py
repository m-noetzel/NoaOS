...
_API_BASE = "https://api.moonshot.cn"
_MAX_RETRIES = 3
_RETRY_STATUSES = {429}
...
class KimiClient:
    """Client for the Kimi (Moonshot AI) Chat Completions API.

    Kimi exposes an OpenAI-compatible interface, so request/response
    formats are identical to OpenAIClient.  Only the base URL and
    ...
    def build_request(
        ...
        *,
        ...
        """Build a request payload for the Kimi Chat Completions API."""
    ...
    def _normalize_messages(
        ...
        """Re-format internal messages to OpenAI-compatible format.

        Converts internal tool_calls ({id, name, input}) to OpenAI
            ...
            if msg.get("role") == "assistant" and msg.get("tool_calls"):
                ...
                normalized.append({
                    **msg,
                    ...
                    # OpenAI-compatible: content must be null (not empty string)
    ...
    async def _send_request(self, request: dict[str, Any]) -> httpx.Response:
        """Send a request to the Kimi API with retry on 429."""
        async with httpx.AsyncClient(
            ...
            for attempt in range(_MAX_RETRIES):
                ...
                delay = 2 ** attempt
                logger.warning(
                    "Kimi %d (attempt %d/%d), retrying in %ds",
    ...
    def _parse_response(self, response: httpx.Response) -> dict[str, Any]:
        """Parse a Kimi API response into normalized format."""
        if response.status_code == 401:
            msg = "Kimi: invalid API key (401)"
        ...
        if response.status_code != 200:
            ...
            msg = f"Kimi API error {response.status_code}: {detail}"
            raise ProviderError(msg)
    ...
    async def complete_stream(
        ...
        *,
        ...
        """Stream a completion request to Kimi, yielding token chunks.
    ...
    async def _stream_request(
        ...
        """Internal async generator that streams from Kimi SSE."""
        ...
        try:
            ...
            msg = "Timeout calling Kimi API (streaming)"
            raise ProviderError(msg) from exc
