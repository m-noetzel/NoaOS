...
| CI-030 | Expand Ruff Gate to Cover tests/ | P1 | **APPLIED** (NOT EFFECTIVE -- pre-push hook never fires in agent pipeline; see CI-050) | tools/pre-push-hook.sh + .github/workflows/ci.yml | 2026-03-12 | 2026-03-12 | W24: 43 violations shipped |
...
| CI-044 | Signal Log Completeness Enforcement | P3 | SUPERSEDED by CI-046 | CLAUDE.md | 2026-03-14 | — | Promoted with updated evidence in W23 analysis |
| CI-045 | Hollow Migration Detection Gate (M7 extension) | P2 | PROPOSED | Plan/QA_CHECKLIST.md | 2026-03-18 | — | — |
| CI-046 | Signal Log Completeness Enforcement (promoted from CI-044) | P2 | PROPOSED | CLAUDE.md | 2026-03-18 | — | — |
| CI-047 | Refactoring Wave Cross-Phase Test Validation | P3 | PROPOSED | CLAUDE.md | 2026-03-18 | — | — |
| CI-048 | Post-Implementation File Verification Gate | P1 | **APPLIED** | CLAUDE.md | 2026-03-20 | 2026-03-20 | — |
| CI-049 | Runtime Data Pipeline Integration Test Requirement (M9) | P2 | **APPLIED** | Plan/QA_CHECKLIST.md | 2026-03-20 | 2026-03-20 | — |
| CI-050 | Ruff Verify Gate for Agent Pipeline (supersedes CI-030 enforcement) | P2 | **APPLIED** | CLAUDE.md | 2026-03-20 | 2026-03-20 | — |

---

## Proposal Details

### Wave 24 Boundary (2026-03-20)

Full analysis: [`Plan/CI/analysis_2026-03-20_wave24.md`](analysis_2026-03-20_wave24.md)

- **CI-048** (P1 -- human gate required): Post-implementation file verification. Implement agent reported success on 5 deliverables without writing code to disk (W24 batch 1 RCA). No existing gate independently verifies file changes. Proposed: orchestrator runs `git diff --stat` after implement agent completes and confirms expected files appear in diff.
- **CI-049** (P2): Runtime data pipeline integration test requirement (new M9 gate). EV1/EV2 evaluator persists `run_id=""` -- tests pass because they seed data directly, masking that the graph never produces valid records. Same pattern in W22 FR5 (CostRecord.run_id type mismatch). Proposed: for phases that persist data for consumption by another component, at least one test must exercise the full producer->consumer path.
- **CI-050** (P2): Ruff verify gate for agent pipeline. CI-030 applied ruff to pre-push hook, but "No GitHub push -- only human pushes" means the hook never fires. W24: 43 violations, W23: 32 violations shipped. Proposed: add `ruff check tests/` to CLAUDE.md Verify gate (where agent execution actually runs).

**Effectiveness confirmed this wave:**
- CI-009 (L12 Write-Path User Scoping): EFFECTIVE -- 6th consecutive wave, no violations.
- CI-013 (M5b Findings Currency): EFFECTIVE -- correctly caught DEV-H1 false resolution. 6th consecutive wave.
- CI-015 (Findings Sync): EFFECTIVE -- sync enforced in W24 QA.
- CI-017 (M8b Cross-Language Field Optionality): EFFECTIVE -- all W24 Pydantic fields correct.
- CI-031 (app.state write-only): EFFECTIVE -- 5th consecutive wave, no violations.
- CI-042 (DB dead-end stores): EFFECTIVE -- 3rd consecutive wave, no new dead-end stores.

**Revised effectiveness:**
- CI-030 (Ruff on tests/): **NOT EFFECTIVE** (revised from EFFECTIVE). Pre-push hook never fires in agent pipeline. W24: 43 violations, W23: 32 violations. Prior "effective" assessment was coincidental (audit-fix phases cleaned tests). CI-050 proposed to fix enforcement point.
- CI-033 (Pre-QA Deliverable Check): **PARTIALLY EFFECTIVE**. Works when agent actually writes code (batch 2 all deliverables present). Fails when agent hallucates completion (batch 1) because it is a self-check, not independent verification. CI-048 proposed to add independent verification.

**Not effective this wave:**
- CI-023 (Pre-Phase Test Plan): NOT EFFECTIVE -- 0% compliance, 6th consecutive wave.

---

### Wave 23 Boundary (2026-03-18)

Full analysis: [`Plan/CI/analysis_2026-03-18_w23.md`](analysis_2026-03-18_w23.md)

- **CI-045** (P2): Hollow migration detection gate. Migration 019 (CQ9) adds `tool_name`/`function_name` columns to approvals but `_create_approval()` never populates them -- columns always NULL. Same pattern as TM2 (Wave 18). Proposed M7 checklist item: when a phase adds a migration with new columns, verify the write path populates them.
- **CI-046** (P2): Signal log completeness enforcement (promoted from CI-044 P3). Wave 22 had 4/6 phases missing. Wave 23 had 9/9 phases missing. 13 total missing rows across 2 consecutive waves. Proposed: mandatory signal log row after every QA verdict, same enforcement class as FINDINGS sync.
- **CI-047** (P3): Refactoring wave cross-phase test validation. Wave 23 broke 31 tests across 16 files from cross-cutting code changes. Tests not updated alongside code. Single occurrence -- placing at P3 with low confidence.

**Effectiveness confirmed this wave:**
- CI-042 (M7 dead-end store broadened): EFFECTIVE -- W22-H1/H2 both wired in CQ1. Zero new dead-end stores.
- CI-031 (M7 app.state write-only): EFFECTIVE -- zero app.state dead-end stores. 3rd consecutive effective wave.
- CI-013 (M5b Findings Currency): EFFECTIVE -- caught W23-audit-fix FINDINGS.md omission on Cycle 1. 5th consecutive wave.
- CI-015 (Findings Sync): EFFECTIVE -- no stale findings at wave close after Cycle 2 fix.
- CI-030 (Ruff on tests/): EFFECTIVE -- ruff clean on tests/ after audit-fix. 3rd consecutive wave.
- CI-009 (L12 Write-Path User Scoping): EFFECTIVE -- no violations. 5th consecutive wave.
- CI-017 (M8b Cross-Language Field Optionality): EFFECTIVE -- all new fields correctly optional.
- CI-033 (Pre-QA Deliverable Check): EFFECTIVE -- no completeness FAIL verdicts.

**Not effective this wave:**
- CI-023 (Pre-Phase Test Plan): NOT EFFECTIVE -- 0/10 phases had a test plan. 3rd consecutive wave at 0% compliance. CI-038 (M1b enforcement gate) remains DEFERRED.
