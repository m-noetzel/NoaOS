...
### M9: Runtime Data Pipeline Test (CI-049)
- [ ] For phases that persist data consumed by another component (e.g., evaluator → analytics), at least one test exercises the full producer→consumer path using the real code path (not seeded DB data)
- [ ] Tests that seed data directly must be accompanied by at least one test that runs the producing code and verifies the consumer reads the output correctly
