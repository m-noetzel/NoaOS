...
| W24-AUDIT-H1 | High | Langfuse env vars not configured in noa-api service — `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_HOST` not in noa-api environment block of `docker-compose.yml` or `docker-compose.dev-full.yml`. All LF1 instrumentation silently no-ops. Langfuse Traces page shows no Noa data. | Open | — |
