...
| TECH-M4 | Medium | Run stuck in "running" on container crash — if the API container crashes mid-graph-execution, the Run row stays in `status="running"` permanently. No crash-recovery mechanism resets orphaned runs on startup. Users see a frozen run that never completes or fails. `LS2` adds a per-request timeout but does not cover crash recovery. **To fix:** on startup, query for runs with `status="running"` older than `timeout_seconds` and mark them `"failed"`. | Open | — |
...
**Open:** 21 | **Partially Resolved:** 0 | **Resolved:** 192 | **Total:** 213
