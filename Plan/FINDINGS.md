...
| W24B-L1 | Low | RLS adoption gap — `set_domain_context()` in `src/noa/db/rls.py` is never called from any API endpoint or service. RLS policies exist in Postgres but provide zero runtime protection since all queries run without an explicit domain context (empty-string bypass = see all rows). The helper is ready; endpoints need to call it. Affects: `/api/v1/chat`, `/api/v1/memory`, `/api/v1/approvals`, `/api/v1/threads`. Intentional design decision per PHASE_DETAILS.md ("callers adopt incrementally") but needs tracking. | Open | — |
...
| W27-L1 | Low | `tests/unit/test_w27_fx2.py` deleted from disk after initial run — only `.pyc` remains. 11 tests for CQ9-M1 approval fix, Ollama temperature default, and dead callback removal are absent. The code fixes are present but have no test protection. Blocking W27 Cycle 2. | Open | — |
...
| OI8-M1 | Medium | OI8 Smart Domain Redirect marked "Complete" in PLAN.md but only test file exists (`tests/unit/test_oi8_domain_redirect.py`) — no implementation in API endpoints or orchestrator. Currently returns 403 on domain mismatch instead of auto-creating thread in correct domain and emitting SSE redirect event. File: `src/noa/api/v1/chat.py`, `src/noa/api/v1/threads.py`. | Open | — |

| RV-M1 | Medium | Idempotency cache is in-memory only — `ToolGateway._idempotency_cache` (gateway.py) and chat endpoint idempotency check (chat.py) use plain Python dicts. Lost on restart, no cross-instance support. Should move to PostgreSQL or Redis (`idempotency_keys` table with periodic cleanup). File: `src/noa/tools/gateway.py`, `src/noa/api/v1/chat.py`. | Open | — |
| RV-M2 | Medium | Privacy classifier relies on static keyword matching only — `_PRIVATE_KEYWORDS` in `src/noa/privacy/classifier.py` is a small hardcoded set (journal, diary, private, personal, secret, password, confidential). Misses "medical history", "resignation letter", multi-turn private context. Should augment with local sentence-transformer classifier on Ollama or at minimum make keywords user-configurable via Settings API. File: `src/noa/privacy/classifier.py`. | Open | — |
| RV-M3 | Medium | No LLM response quality evaluation test suite — all LLM-touching tests mock provider responses. No golden prompts, no fixture-based structural assertions, no offline evaluation suite. Prompt regressions or tool-calling failures in production go undetected. Should create recorded-response test suite (vcrpy cassettes or YAML fixtures) asserting structural properties (e.g. tool_calls non-empty for action prompts, privacy_mode=private for sensitive prompts). File: `tests/`. | Open | — |

**Open:** 31 | **Partially Resolved:** 0 | **Resolved:** 218 | **Total:** 250
