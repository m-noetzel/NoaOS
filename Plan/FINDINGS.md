...
| ADHOC-2026-03-17-M1 | Medium | No regression tests for temperature wiring or `[TASK_COMPLETE]` contract — wiring works but has no test protection | Open | — |
| ADHOC-2026-03-17-M2 | Medium | `OllamaClient.complete` temperature param is `float = 0.7` (always set) vs `float | None = None` on all other providers — inconsistent, may override user intent | Open | — |
| CQ9-M1 | Medium | Approval `tool_name`/`function_name` columns added by migration 019 but never populated — `_create_approval()` in `chat.py:875-883` constructs `Approval()` without `tool_name=tool, function_name=function`. Columns always NULL in production. CQ9 deliverable "replace string-split parsing" is structurally incomplete. | Open | — |
| UI-H6 | High | UI does not adapt to privacy domain — switching to private mode has no visual effect on any view (Settings, Threads, Runs, Cost, Tools, Memory, Approvals, Queue, Artifacts). All data shown unfiltered, external providers selectable in private mode. Router classifier heuristics are dead code. | Open | — |
...
| W25-L2 | Low | BU1 `check-bundle-size.sh` uses integer division `$(( bytes / 1024 ))` for KB calculation. Truncation could cause boundary-value confusion at the 500KB gate. | Open | -- |
...
**Open:** 15 | **Partially Resolved:** 0 | **Resolved:** 204 | **Total:** 220
