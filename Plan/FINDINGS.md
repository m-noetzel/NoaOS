...
| W24-M5 | Medium | `_handle_rag_ingest` in `src/noa/private_worker/handlers.py:125` calls `_memory_store.store()` without the required `source_thread_id: str` argument. Causes `TypeError` at runtime when a client calls `rag_ingest`. Fix: add `source_thread_id=payload.get("source_thread_id", "")` to the store() call. | Open | — |
| W24-M6 | Medium | `IdempotencyKey` ORM model missing from `src/noa/db/models/__init__.py`. `Base.metadata` does not include `idempotency_keys` table. Any test or code path using `Base.metadata.create_all()` (instead of alembic migrations) will silently omit the table. Also causes ruff I001 import-sort violation due to `MemoryFact` being appended out of order at bottom of file. Fix: add `from noa.db.models.idempotency_key import IdempotencyKey  # noqa: E402, F401` in alphabetical order. | Open | — |
...
**Open:** 19 | **Partially Resolved:** 0 | **Resolved:** 196 | **Total:** 215
