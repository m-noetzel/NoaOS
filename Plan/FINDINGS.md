...
| AUTH-H1 | High | `apiRequest` 401-handler fires on login endpoint — credential failures shown as "Session expired" | **Resolved** | AU1 |
...
| ADHOC-2026-03-17-H1 | High | `[TASK_COMPLETE]` marker never instructed in system prompt — run lifecycle broken, runs never transition to "completed" status (pre-existing since 5bccc62) | **Resolved** | ADHOC-fix |
| ADHOC-2026-03-17-H2 | High | System prompt file not mounted in production container — `prompts/` dir missing from docker-compose volumes, backend read/wrote ephemeral container-local copy diverging from host | **Resolved** | ADHOC-fix |
| ADHOC-2026-03-17-H3 | High | Chat composer `onBlur` silently overwrote global system prompt — editing prompt in chat advanced settings PATCHed the canonical file, causing Settings page desync | **Resolved** | ADHOC-fix |
| ADHOC-2026-03-17-H4 | High | Temperature slider not wired to backend — UI sent value but it was dropped at 3 points: `agent_node`, external worker `/v1/complete`, and OpenAI client `build_request()` | **Resolved** | ADHOC-fix |
| ADHOC-2026-03-17-M1 | Medium | No regression tests for temperature wiring or `[TASK_COMPLETE]` contract — wiring works but has no test protection | **Resolved** | W27-FX3 |
| ADHOC-2026-03-17-M2 | Medium | `OllamaClient.complete` temperature param is `float = 0.7` (always set) vs `float | None = None` on all other providers — inconsistent, may override user intent | **Resolved** | W27-FX2 |
| CQ9-M1 | Medium | Approval `tool_name`/`function_name` columns added by migration 019 but never populated — `_create_approval()` in `chat.py:875-883` constructs `Approval()` without `tool_name=tool, function_name=function`. Columns always NULL in production. CQ9 deliverable "replace string-split parsing" is structurally incomplete. | **Resolved** | W27-FX2 |
| UI-H6 | High | UI does not adapt to privacy domain — switching to private mode has no visual effect on any view (Settings, Threads, Runs, Cost, Tools, Memory, Approvals, Queue, Artifacts). All data shown unfiltered, external providers selectable in private mode. Router classifier heuristics are dead code. | **Resolved** | W27-FX1 |
| W23-H1 | High | 25 persistent test failures — test files not updated to match Wave 23 (CQ1-CQ9) code changes: runner event sequence (ainvoke→astream), model defaults (openai/gpt-4.1-mini), settings schema, Notion create_page API, risk tier reclassifications, auth service contract. CI would fail on push. | **Resolved** | W23-audit-fix |
| W23-H2 | High | Test isolation pollution — ~29 order-dependent failures from global app_state not reset between test files. Tests that pass in isolation fail in full suite. `reset_all()` fixture missing. | **Resolved** | W23-audit-fix |
| W23-M1 | Medium | 8 ruff E501 violations in src/ — auth.py:411,413, memory.py:73, openai.py:80, runner.py:231,268, direct.py:26, definitions.py:221. CI gate violation. | **Resolved** | W23-audit-fix |
| W23-M2 | Medium | 32 ruff errors in tests/ — 27 unused imports auto-fixable + 5 manual violations (SIM117, SIM105, S107, S110). Test hygiene failure. | **Resolved** | W23-audit-fix |
| TECH-H1 | High | No LLM token streaming — all provider clients (Anthropic, OpenAI, Google AI, Ollama) buffer the complete response before returning. Users see a full delay between "thinking" and text appearing; no incremental token delivery. SSE streams orchestration steps but not LLM output tokens. See Tech.md §Weak Points #2. **To fix:** stream=True in each provider client, yield `token` SSE events from runner. **LS1 implemented streaming infrastructure** (complete_stream() on all providers, runner token_stream events, agent node callback) — but token delivery was post-node-completion rather than concurrent (W24-M1). **TS1** fixed concurrent delivery: token callback now puts events directly into a shared queue drained by the runner's main generator alongside the graph task. | **Resolved** | TS1 |
| TECH-M1 | Medium | JWT tokens non-revocable on logout — logout invalidates the DB session but issued JWTs remain valid for up to 7 days. A stolen token cannot be invalidated server-side. See Tech.md §Weak Points #3. **To fix:** token blacklist table (checked on every request) or switch to 15-min access tokens + refresh. | **Resolved** | SEC1 |
| TECH-L1 | Low | Privacy classifier uses keyword substring matching only — "journal", "diary", "private", "secret" matched as substrings in the most recent user message only; ignores conversation context. False positives: "private company", "diary industry". False negatives: multi-turn private conversations lose context. See Tech.md §Weak Points #5. Acceptable for single-user (fail-safe defaults to private), but production-quality needs improvement. | Open | — |
| TECH-M2 | Medium | No orchestrator-level timeout watchdog — the LangGraph graph has no built-in execution timeout. Only per-request httpx client timeouts (60s) protect against hung LLM providers. A stalled tool or hung graph node can hold a request indefinitely. See Tech.md §Weak Points #9. **To fix:** asyncio.wait_for wrapper around graph execution in runner.run(), configurable via settings `timeout_seconds`. | **Resolved** | LS2 |
| TECH-M3 | Medium | Egress control is advisory only — the `noa.egress.allowlist` Docker label on external-worker declares intended allowed hosts but Docker does not enforce it natively. No external proxy (squid, mitmproxy) is configured. The external worker can reach any internet host. See Tech.md §Weak Points #10. Requires Phase 2 infrastructure (external egress proxy). Deferred. | Open | — |

| TECH-M4 | Medium | Run stuck in "running" on container crash — if the API container crashes mid-graph-execution, the Run row stays in `status="running"` permanently. No crash-recovery mechanism resets orphaned runs on startup. Users see a frozen run that never completes or fails. `LS2` adds a per-request timeout but does not cover crash recovery. **To fix:** on startup, query for runs with `status="running"` older than `timeout_seconds` and mark them `"failed"`. | **Resolved** | ST1 |
| TECH-L2 | Low | Tool definition fragmentation — tool metadata is split across three files with no single registry: `definitions.py` (function schemas/signatures), `capabilities.py` (permission model), `registration.py` (domain, auth model). Adding or modifying a tool requires coordinated changes in all three. No compile-time enforcement that they stay in sync. Acceptable now but will cause drift as the tool count grows. | Open | — |
| CHAT-H1 | High | Email send confirmation loop — when user requests "send email", LLM calls `draft_email` (low-risk, no approval) instead of `send_email` (medium-risk, triggers ApprovalCard), then asks conversationally for confirmation. On the confirmation turn, the LLM generates text instead of calling `send_email` because tool call results from the previous turn are not persisted in conversation history. The LLM has no structured context (to/subject/body) to reconstruct the call. Root fix: persist tool_calls + tool_results as messages in the DB and include them in the history passed to the LLM on subsequent turns. | **Resolved** | ST2 |
| CHAT-H2 | High | Runs permanently stuck in "running" — two causes: (1) After approval, `_execute_approved_tool` sets run status to `"running"` but never finalises to `"completed"` or `"failed"` after the tool executes. (2) Normal runs only reach `"completed"` if the LLM emits `[TASK_COMPLETE]` marker (chat.py:386), which it never does since the system prompt does not instruct it to. Fix: replace LLM-marker-based completion with system-based goal completion — derive goal from first message, mark `"completed"` when tool calls succeed or a response is generated without errors, mark `"failed"` when all tool calls fail or no response is produced. | **Resolved** | ST1 |
| RV1-H1 | High | Auto-privacy-classifier silently overrides explicit `privacy_mode` from client — `submit_chat` in `chat.py:131-159` runs `PrivacyClassifier().classify()` on new-thread messages even when the client sends `privacy_mode="external"`. If the classifier returns "private" (e.g. message contains "private", "secret", etc.), the explicit client intent is discarded and the request is queued when the private worker is unavailable. Breaks Transparency Principle ("no silent overrides"). Causes regression in `test_queue_wiring.py::TestExternalPath`. Fix: only run classifier when `body.privacy_mode is None`. | **Resolved** | RV1 (cycle 2) |
| RV1-M1 | Medium | `tool_start`/`tool_end` added to `VALID_EVENT_TYPES` without updating iOS3 contract test — `test_ios3_networking_contract.py::TestValidEventTypes::test_backend_event_types_match_spec` uses equality assertion against a hardcoded set; adding new event types to `schemas.py` now fails this test. Fix: add `tool_start`/`tool_end` to `REQUIRED_EVENT_TYPES` in the test class and document these as valid iOS SSE event types. | **Resolved** | RV1 (cycle 2) |
| RV1-M2 | Medium | `buildCostItems()` exported but untested — the rewritten function in `CostBreakdown.tsx` has no tests. Critical paths untested: multi-call `llm_usage` array, empty `llm_usage` with `total_cost > 0` fallback, missing `cost_usd` → estimateCost fallback, `run.steps` canonical path. Per CI-012/S5b, source changes require behavioral companion tests. | **Resolved** | RV1 (cycle 2) |

| DEV-L1 | Low | `docker-compose.dev-full.yml` missing `TZ` env var on `noa-api-dev` — container ran UTC, causing `_make_datetime_entry` to attach `+00:00` offset to naive datetime strings from LLM. Calendar events created 1 hour late in CET. Resets on every container restart. **Fixed:** Added `TZ=${TZ:-Europe/Berlin}` to `noa-api-dev` environment in `docker-compose.dev-full.yml`. | **Resolved** | 2026-03-19 hotfix |
| W24-H2 | High | EV1 evaluator persists all records with `run_id=""` — EV2 analytics permanently broken at runtime. `evaluator_node()` in `src/noa/orchestrator/nodes/evaluator.py:227` passes `run_id=""` to `_persist_evaluation()` because `run_id` is absent from `AgentState`. All auto-evaluation rows are orphaned. `GET /analytics/eval-trends` and `GET /analytics/worst-dimensions` return empty data for all users in production. Fix: add `run_id: str \| None` to `AgentState` TypedDict; populate in `runner.py` initial_state; read in `evaluator_node()`. | **Resolved** | ST3 |
| DEV-H1 | High | External worker had no `/data` volume mount in `docker-compose.yml` or `docker-compose.dev-full.yml`. `MemoryStore(data_dir=Path("/data/memory/external"))` wrote to the ephemeral container layer — all external memory facts were lost on every container restart. Private worker already had `private-data:/data`. | **Resolved** | W23-FIX |
| W24-H1 | High | VM1 VectorMemoryStore is dead code — `VectorMemoryStore` class implemented, `memory_facts` table created by migration 022, OllamaClient.embed() implemented, 20 tests pass. But the class is never instantiated in app startup (not in app.py lifespan). Private worker handlers `_handle_rag_query` and `_handle_rag_ingest` remain stubs returning `{"status": "queried", "results": []}`. Phase plan deliverable "Wire rag_ingest, rag_query, summarize, search RPC handlers in private worker" not completed. VectorMemoryStore cannot be reached from any running code path. File: `src/noa/private_worker/handlers.py:99-106`. | **Resolved** | W24-batch1-c3 |
| W24-M1 | Medium | LS1 token streaming is post-node, not concurrent — tokens are queued during LLM execution via `_token_cb` callback but drained only AFTER the agent node completes (`while not token_queue.empty()`). This delivers tokens in a burst rather than incrementally, functionally equivalent to non-streaming with extra complexity. True streaming requires concurrent queue draining alongside `astream()`. File: `src/noa/orchestrator/runner.py:256-265`. | **Resolved** | ST4 (partially — per-run callback isolation fixed; burst delivery is an accepted limitation, deferred to future work) |
| W24-M2 | Medium | LS1 module-level `_stream_callback` is a process-global — `_stream_callback` in `src/noa/orchestrator/nodes/agent.py` is module-level state. Concurrent requests on the same event loop share this callback. Request A's LLM tokens can be routed to Request B's SSE queue if they overlap. Current single-user deployment is not affected, but this is a latent defect for future multi-user use. | **Resolved** | ST4 |
| W24-M3 | Medium | CX1 gateway.py mypy error — `sweep_idempotency_keys()` returns `result.rowcount` which SQLAlchemy types as `Any`, causing `no-any-return` mypy error at `src/noa/tools/gateway.py:208`. Verified resolved: `mypy src/noa/tools/gateway.py` now returns 0 errors (int() cast applied). | **Resolved** | W24-batch1-c3 |
| W24-M4 | Medium | VM1 missing required deliverables — phase plan PHASE_DETAILS.md VM1 Files table lists two files not created: (1) `src/noa/private_worker/embeddings.py` (OllamaEmbedder class) and (2) `tests/integration/test_vm1_pgvector.py` (pgvector integration test). The inline `_get_embedding()` in handlers.py partially covers the OllamaEmbedder responsibility but is not the encapsulated class the spec requires. Acceptance criterion 6 (≥1 integration test using real Postgres + pgvector) is unmet. | **Resolved** | ST5 |
| W24-M5 | Medium | `_handle_rag_ingest` in `src/noa/private_worker/handlers.py:125` calls `_memory_store.store()` without the required `source_thread_id: str` argument. Causes `TypeError` at runtime when a client calls `rag_ingest`. Fix: add `source_thread_id=payload.get("source_thread_id", "")` to the store() call. | **Resolved** | ST5 |
| W24-M6 | Medium | `IdempotencyKey` ORM model missing from `src/noa/db/models/__init__.py`. `Base.metadata` does not include `idempotency_keys` table. Any test or code path using `Base.metadata.create_all()` (instead of alembic migrations) will silently omit the table. Also causes ruff I001 import-sort violation due to `MemoryFact` being appended out of order at bottom of file. Fix: add `from noa.db.models.idempotency_key import IdempotencyKey  # noqa: E402, F401` in alphabetical order. | **Resolved** | ST5 |
| W24-AUDIT-H1 | High | Langfuse env vars not configured in noa-api service — `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_HOST` not in noa-api environment block of `docker-compose.yml` or `docker-compose.dev-full.yml`. All LF1 instrumentation silently no-ops. Langfuse Traces page shows no Noa data. | **Resolved** | W24B-FIX |
| W24-AUDIT-H2 | High | Test fixture `_make_agent_state()` in `tests/unit/test_orchestrator.py` missing `run_id` field added by EV1. Causes `test_state_factory_produces_valid_state` to fail — CI gate regression. | **Resolved** | W24B-FIX |
| W24-AUDIT-H3 | High | 43 ruff errors in Wave 24 test files (I001 import sorting, F401 unused imports) — CI ruff gate would fail on push. All auto-fixable. | **Resolved** | W24B-FIX |
| W24B-L1 | Low | RLS adoption gap — `set_domain_context()` in `src/noa/db/rls.py` is never called from any API endpoint or service. RLS policies exist in Postgres but provide zero runtime protection since all queries run without an explicit domain context (empty-string bypass = see all rows). The helper is ready; endpoints need to call it. Affects: `/api/v1/chat`, `/api/v1/memory`, `/api/v1/approvals`, `/api/v1/threads`. Intentional design decision per PHASE_DETAILS.md ("callers adopt incrementally") but needs tracking. | **Resolved** | DI3 |
| W25-L1 | Low | `TokenBlacklist` import inserted out of alphabetical order in `src/noa/db/models/__init__.py` — placed after `ToolCallLog` (line 25) but alphabetically `token_blacklist` sorts before `tool_call_log`. Causes ruff I001 violation. Auto-fixable with `ruff check --fix`. Introduced by SEC1. | **Resolved** | W25-FIX |
| W25-H1 | High | IS1 SendMessageIntent SSE parsing uses wrong JSON keys (`event["event"]` instead of `event["event_type"]`, `event["data"]` instead of `event["payload"]`). Intent collects zero SSE events and always returns empty/fallback response. Siri shortcut is non-functional. File: `ios/Noa/Sources/Noa/Intents/SendMessageIntent.swift:122-144`. | **Resolved** | W25-FIX |
| W25-M1 | Medium | KIMI_API_KEY not wired to docker-compose files. Users who set the env var on host will not have it propagated into the noa-api container. Kimi provider never initializes in Docker deployment. Recurring pattern (same as Wave 20 Google OAuth, Wave 24 Langfuse). | **Resolved** | W25-FIX |
| W25-M2 | Medium | Kimi and Google AI models missing from `PRICING_TABLE` in `src/noa/cost/pricing.py`. `estimate_cost()` returns $0 for all Kimi/Google AI usage. Cost dashboard under-reports spending; budget enforcement does not account for these providers. | **Resolved** | W25-FIX |
| W25-M3 | Medium | SEC1 blacklist check fails open on DB error in `require_auth()`. A revoked token can authenticate during a DB outage window. Documented trade-off for availability (personal system). Warning logged. No immediate fix needed but should be in operational runbook. | Open | -- |
| W25-L2 | Low | BU1 `check-bundle-size.sh` uses integer division `$(( bytes / 1024 ))` for KB calculation. Truncation could cause boundary-value confusion at the 500KB gate. | **Resolved** | W27-FX3 |
| W25B-M1 | Medium | `async with contextlib.suppress()` in `app.py:444` — `contextlib.suppress` is sync-only, has no `__aenter__`/`__aexit__`. Will raise `AttributeError` at shutdown when cancelling the blacklist cleanup task. mypy catches this. | **Resolved** | W25B-FIX |
| W25B-M2 | Medium | `sse_types.py` TokenEvent TypedDict defines `event_type: Literal["token"]` with `content` payload, but runner.py emits `"token_stream"` with `token` payload. `VALID_SSE_EVENT_TYPES` includes `"token"` not `"token_stream"`. Stale type definitions; no runtime impact since TypedDicts are not enforced. | **Resolved** | W25B-FIX |
| W25B-L1 | Low | Kimi context windows missing from `MODEL_CONTEXT_WINDOWS` in `src/noa/orchestrator/token_budget.py`. Kimi models (e.g. `moonshot-v1-128k`) default to the 4096-token fallback, causing incorrect token budget calculations and premature compaction boundaries for Kimi users. Fix: add Kimi model entries with 131072-token windows. | **Resolved** | ST5 |
| W26-M1 | Medium | `_extract_turn_messages()` in `chat.py` collapses all tool calls across multiple tool-call rounds in one turn into a single assistant message. If the LLM makes two separate tool rounds (tool_called → tool_result → tool_called → tool_result in one SSE stream), both rounds' calls are bundled into one assistant message. This is semantically incorrect for multi-round tool use and can corrupt LLM history on subsequent turns. Also: providers that omit `id` on tool calls (e.g. Ollama, some Kimi responses) cause the second result to overwrite the first when the same tool is called twice (key collision). File: `src/noa/api/v1/chat.py:_extract_turn_messages()`. | **Resolved** | DI1 |
| W26-L1 | Low | ST1 integration test (`test_st1_integration.py`) re-implements the orphan recovery SQL logic inline instead of calling `_recover_orphaned_runs()` from `app.py`. If the `await _recover_orphaned_runs()` call is accidentally removed from the lifespan, all ST1 integration tests still pass. Violates the "integration test exercises real code path" principle. Fix: call `_recover_orphaned_runs()` directly in the test, or test via ASGI lifespan. | **Resolved** | W27-FX3 |
| W26-L2 | Low | W25B-L1 (Kimi context windows) is referenced in phase plans and test files but has no formal entry in the FINDINGS.md Tracking Summary. The finding was closed implicitly by ST5. Add a row and mark resolved for completeness. | **Resolved** | ST5 |
| W26-L3 | Low | Dead module-global streaming callback in `agent.py`. `_stream_callback`, `set_stream_callback()`, `get_stream_callback()` are defined but never called after ST4 migrated to per-run `token_callback` in state. Line 318 fallback `or _stream_callback` is always None. Cleanup opportunity. | **Resolved** | W27-FX2 |
| W27-L1 | Low | `tests/unit/test_w27_fx2.py` deleted from disk after initial run — only `.pyc` remains. 11 tests for CQ9-M1 approval fix, Ollama temperature default, and dead callback removal are absent. The code fixes are present but have no test protection. Blocking W27 Cycle 2. | **Resolved** | W29 |
| W27-L2 | Low | Two ThreadSidebar domain filter tests fail (`web/src/test/w27-fx1-domain-ui.test.tsx:146,172`). Mock returns same thread data for both `privacy_mode=private` and `privacy_mode=external` queries; filter tests cannot differentiate results. Component logic is correct; test mock is incorrect. Blocking W27 Cycle 2. | Open | — |
| W27-L3 | Low | `web/scripts/check-bundle-size.sh:49` gate comparison still uses integer division `$(( gz_bytes / 1024 ))` after the W25-L2 fix. Display values use float via awk but the actual gate threshold check truncates. A file at exactly 500.1 KB gzip passes the gate silently. W25-L2 is only half-fixed. Blocking W27 Cycle 2. | Open | — |
| UX-EV1 | Medium | Evaluator thresholds (pass ≥3.0, reroute ≥2.0, max cycles) are hardcoded in `src/noa/orchestrator/nodes/evaluator.py`. No UI to view or adjust them. Settings page should expose evaluator thresholds (pass_threshold, reroute_threshold, max_reroute_cycles) so the user can tune quality gates without code changes. | **Resolved** | OV4 |
| BE-AP1 | High | Run summary not updated after approval decision. The `summary` field is computed from SSE events during the run and frozen at "Awaiting approval for \<tool\>". After the user approves/denies, the summary is never re-computed to reflect the final outcome (e.g. tool result, completion). The run detail page shows stale "Awaiting approval" text indefinitely. File: `src/noa/api/v1/approvals.py` + `src/noa/api/v1/chat.py:_compute_summary()`. | Open | — |
| UX-RG1 | Medium | Execution Graph node detail panel shows no content. Clicking a node (e.g. Planner) in the RunDetail Execution Graph shows only the node label and replay buttons — no input/output data. The planner's `plan` field, classifier's `task_type`, router's `selected_model` etc. are not displayed. Node output data is available in events but not rendered in the `RunGraph` component. File: `web/src/components/runs/RunGraph.tsx`. | **Resolved** | UX1 |
| BE-CP1 | High | Checkpointer crashes on non-serializable state fields. The `token_callback` function in `AgentState` caused `TypeError: Object of type function is not JSON serializable` when `PostgresCheckpointer.save()` tried to persist the state as JSON. This crashed the entire run after the LLM had already produced a response. Fixed: non-serializable keys are now stripped before saving. File: `src/noa/orchestrator/checkpointer.py`. | **Resolved** | ADHOC-fix |
| ARCH-AP1 | High | Approval flow runs outside graph — no interrupt, no Langfuse coverage. When a tool needs approval, the graph runs to completion (responder → evaluator → end) with a placeholder "awaiting approval" response. The actual tool execution after approval happens in `_execute_approved_tool()` (`approvals.py:263`), which is a standalone API handler — no LangGraph node, no Langfuse trace, no evaluation. This causes: (1) evaluator reroutes on the placeholder response (sinnloser extra Zyklus), (2) tool execution is invisible in Langfuse, (3) response quality is never evaluated for the real result. Fix: use LangGraph `interrupt()` in tools node so the graph pauses at approval and resumes after decision. Requires LangGraph-native checkpointer (`MemorySaver` or `langgraph-checkpoint-postgres`). Files: `src/noa/orchestrator/nodes/tools.py`, `src/noa/orchestrator/runner.py`, `src/noa/api/v1/approvals.py`, `src/noa/api/v1/chat.py`, `src/noa/api/app.py` (graph compilation). | **Resolved** | OV2 |
| BE-AP2 | High | Evaluator reroutes on approval placeholder response. When `approval_required`, the responder generates a weak holding message (e.g. "I completed the requested actions... but wasn't able to formulate a summary"). The evaluator scores this with completeness=2, actionability=2 → verdict "reroute", triggering a full extra agent → responder → evaluator cycle that wastes LLM tokens and produces no value. Langfuse trace confirms: two evaluator runs, first reroute (2.8/5), then pass (3.0/5). Root cause: evaluator has no awareness of approval state. Quick fix: skip evaluation when `approval_required` in tool_results. Proper fix: ARCH-AP1 (graph interrupt). File: `src/noa/orchestrator/nodes/evaluator.py`, `src/noa/orchestrator/graph.py:route_after_evaluator`. | **Resolved** | OV2 |
| BE-AP3 | Medium | Google OAuth token refresh fails with 400 after approval. When a tool is executed post-approval via `_execute_approved_tool()`, the `GoogleCalendarClient` uses in-memory tokens loaded at app startup. If the access token expired between startup and approval, it attempts a refresh — but Google returns 400 (invalid_grant). Docker logs confirm: 401 from `gmail.googleapis.com` → refresh to `oauth2.googleapis.com` → 400. Likely cause: refresh token revoked/expired. No graceful recovery — error propagates as "Token refresh failed: 400" to UI. Should: (1) reload tokens from DB before execution, (2) surface "please re-connect Google" in UI instead of raw error. File: `src/noa/tools/google_auth.py:198-231`, `src/noa/api/v1/approvals.py:263-313`. | Open | — |

| OBS-LF1 | Medium | Langfuse spans have multiple logging defects. (1) **Node spans log identical Input/Output** for all nodes: runner.py:308-326 filters both from `node_output` (node return value only). No separate input capture. Router, classifier, planner, agent all show same data for Input and Output. Fix: snapshot accumulated `result` state before `result.update(node_output)` as span input. (2) **Tool spans lose error/approval data**: runner.py:370-377 logs `output: {result: tr.get("result", "")}` but approval_required, error, risk_tier are top-level keys on the tool_result dict, not under `result`. Approval tool calls show `output: {result: ""}` in Langfuse — the actual outcome (approval_required, error message) is invisible. Fix: log the full `tr` dict as output, or at minimum include `error` and `approval_required`. (3) **Graph topology invisible**: routing decisions (`route_after_tools`, `route_after_agent`, `route_after_evaluator`) are not logged — you can only infer reroutes from node ordering. No cycle tagging (which `node/agent` is cycle 1 vs 2?), no routing reason spans, no parent-child hierarchy. A reroute looks identical to normal flow in the flat span list. Fix: add routing decision spans, tag nodes with cycle number, use Langfuse parent-child spans to group cycles. File: `src/noa/orchestrator/runner.py:307-377`. | **Resolved** | OV1 |

| PERF-CL1 | Medium | Classifier wastes LLM call on trivial messages. For greetings like "Hallo" or "Hi", the classifier makes a full LLM call (gpt-4o-mini) to classify as `simple_utility` — the result is obvious from the message itself. A cheap heuristic (short message, no verb, greeting pattern) could return `simple_utility` without an LLM call, saving ~200ms + tokens. Additionally, the graph topology is fixed (router → classifier → planner → agent → responder → evaluator) with no conditional shortcut — even when the classifier correctly identifies `simple_utility`, the message still traverses every node. A conditional edge skipping planner (already a no-op for simple_utility) would be cosmetic, but skipping the classifier LLM call for obvious cases is a real cost/latency win. File: `src/noa/orchestrator/nodes/classifier.py`, `src/noa/orchestrator/graph.py`. | **Resolved** | OV5 |

| PERF-PL1 | Medium | Planner node wastes LLM call on simple execution tasks. For `execution` tasks like "Create a meeting at 8pm", the planner makes an LLM call to generate a 4-step plan that restates the obvious ("1. Access the calendar... 2. Set the duration..."). The plan is injected into the agent's system prompt but adds no value — GPT-4.1 already knows to call `calendar__create_event`. This is 1 extra LLM call (~200ms + tokens) on every non-simple_utility request. Only `research` and `decision_intelligence` tasks benefit from planning. Better design: make planning a tool the agent can invoke when needed (complex multi-step tasks), not a mandatory graph node. This would eliminate the LLM call for ~80% of requests. Currently the planner skips only for `simple_utility`; it should also skip for `execution`. File: `src/noa/orchestrator/nodes/planner.py`, `src/noa/orchestrator/nodes/agent.py:244-288` (plan injection). | **Resolved** | OV5 |

| ARCH-RS1 | Medium | Responder node is a fragile fallback layer that produces false information. The responder is not an LLM — it's a pure Python function that picks up `state["response"]` or generates hardcoded fallback strings. When the agent made tool_calls but produced no text (normal for tool-calling turns), the responder says "I completed the requested actions using {tool_names}" — which is false when the tool returned `approval_required` or an error. These fallbacks hide real errors behind friendly messages, trigger unnecessary evaluator reroutes (BE-AP2), and are invisible in Langfuse (text doesn't come from any LLM). The cost summation (only useful part) could be done in the runner. Better design: if the agent has no response after tools, route back to agent (already the normal loop); if agent still empty, surface an explicit error instead of lying. File: `src/noa/orchestrator/nodes/responder.py`. | **Resolved** | OV3 |
| ARCH-EV1 | Medium | Evaluator reroute injects fake user message into conversation. On reroute, the evaluator appends feedback as `role: "user"` (evaluator.py:265-270), impersonating the real user. The agent cannot distinguish evaluator feedback from actual user input. If messages are persisted, the conversation history is corrupted with messages the user never wrote. Langfuse shows it as user input, misleading during debugging. Should use `role: "system"` or `role: "developer"` to clearly identify internal feedback. File: `src/noa/orchestrator/nodes/evaluator.py:264-270`. | **Resolved** | OV4 |
| ARCH-EV2 | High | Evaluator rubric has no dimension definitions, examples, or calibration. The evaluator prompt (evaluator.py:44-65) sends only dimension names (e.g. `goal_alignment`, `completeness`) with a generic 0-5 scale. No per-dimension definition of what each score level means, no anchor examples, no task-type-specific criteria. The LLM must guess what "completeness=2" vs "completeness=3" means — and this 1-point difference decides reroute (< 3.0) vs pass (≥ 3.0). Result: scores are unstable and non-reproducible across runs. An `execution` task (single tool call) is evaluated with the same rubric as a `research` task (multi-source synthesis). The `reasoning` field is requested but never logged or used. Fix: add per-dimension definitions with anchor examples, differentiate rubrics by task_type, log reasoning to Langfuse. File: `src/noa/orchestrator/nodes/evaluator.py:44-65`. | **Resolved** | OV4 |
| ARCH-AG1 | Medium | Agent has no feedback loop — evaluator scores are a dead-end store. The evaluator writes scores to `response_evaluations` table but the agent never reads them. On reroute, feedback is injected as a synthetic "user" message (evaluator.py:265-270) but the agent doesn't know it's from the evaluator. Between runs: no access to prior eval scores, no proactive memory recall, no awareness of approval outcomes, no reflection on cost/errors. The self-improvement flywheel (EV1→EV2) is structurally incomplete — evaluation happens but learning doesn't. The agent should: (1) proactively recall relevant memories before responding, (2) see summarized eval history for recurring topics, (3) learn from approval outcomes (what got approved/denied). File: `src/noa/orchestrator/nodes/agent.py`, `src/noa/orchestrator/nodes/evaluator.py`, `src/noa/db/models/response_evaluation.py`. | Open | — |
| PERF-AP1 | Low | Approval check could short-circuit before Gateway dispatch. Whether a tool needs approval is a static policy decision based on function name + `approvals_enabled` flag — it doesn't depend on arguments or runtime state. Currently the check happens inside `gateway.dispatch()` (gateway.py:308-342), meaning the full Tools-Node dispatch path runs (request construction, adapter lookup, telemetry setup) before discovering approval is needed. The check could be lifted earlier — e.g. a pre-dispatch check in the tools node or a conditional edge — to avoid entering the gateway at all. Low priority since the gateway check itself is fast (no network call), but would make the Langfuse trace cleaner and reduce unnecessary code paths. Related: ARCH-AP1 (proper interrupt would eliminate this entirely). File: `src/noa/tools/gateway.py:308-342`, `src/noa/orchestrator/nodes/tools.py`. | **Resolved** | OV2 |

| UX-EX1 | Medium | Execution tasks produce unnecessary intermediate chat messages. For simple execution tasks ("create a meeting"), the user wants only: (1) success confirmation with details, (2) actionable error message, or (3) approval UI (no chat message needed). Currently the agent paraphrases the request back ("I am scheduling a meeting titled 'Training'...") before the tool even executes — the user already knows what they asked for. With the approval flow, this is worse: the user sees a holding message, then an approval dialog, then potentially an error. The agent should be silent until it has a result to report. For approval: only show the approval UI, no chat bubbles. For success: confirm with actual result data. For error: show a human-readable error with next steps (e.g. "please re-connect Google"), not raw internal errors ("Token refresh failed: 400"). Related: ARCH-AP1 (interrupt eliminates the holding message), ARCH-RS1 (responder generates the false intermediate). File: `src/noa/orchestrator/nodes/agent.py`, `src/noa/orchestrator/nodes/responder.py`. | Open | — |
| BUG-PL2 | Low | Plan is injected twice into system prompt on evaluator reroute. When the evaluator triggers a reroute, the agent node runs again and re-injects the plan into the system message (agent.py:248-288). But the plan was already injected in the first agent run and is still in the messages list. Result: the system prompt contains the same 4-step plan twice, wasting input tokens. Visible in Langfuse trace for the second agent span. File: `src/noa/orchestrator/nodes/agent.py:248-288`. | Open | — |

| OBS-LF2 | High | GENERATION spans have zero tokens and empty input/output. The runner logs LLM generations (runner.py:500-517) with `input_messages=[]` and `output=""` hardcoded. Token counts are always 0 because of a key mismatch: agent.py stores `input_tokens`/`output_tokens` in llm_usage, but runner.py reads `prompt_tokens`/`completion_tokens`. Langfuse GENERATION spans show "0 prompt → 0 completion" for every LLM call, making cost tracking and token analysis useless in Langfuse. Fix: (1) align key names (`input_tokens` everywhere or map both), (2) capture actual messages and output at generation time instead of post-hoc from llm_usage. File: `src/noa/orchestrator/runner.py:500-517`, `src/noa/orchestrator/nodes/agent.py:330-331`. | **Resolved** | OV1 |
| MEM-H1 | High | Agent never proactively stores memory — system prompt `prompts/system_prompt.txt:84` says "Storage (STRICT)" with high barriers ("stable", "likely useful in future"), tool description in `runner.py:660` says "Use remember when the user explicitly asks". The LLM interprets these literally and never triggers `auto_extract` or `remember` unless the user says "remember this". The agent is not self-learning. Fix: (1) change system prompt memory section from STRICT to PROACTIVE — encourage storing any personal fact, preference, habit, date, project context. (2) Change tool description from "explicitly asks" to "whenever the user shares anything worth remembering". File: `prompts/system_prompt.txt:79-97`, `src/noa/orchestrator/runner.py:655-664`. | **Resolved** | MEM1 |
| MEM-H2 | High | No automatic memory recall at conversation start — agent never calls `memory__recall` proactively. System prompt says "Check memory" (line 20) but this relies entirely on the LLM deciding to call the tool, which it never does — it answers directly instead. User context from prior sessions is completely lost between conversations. Fix: either (a) instruct the LLM more forcefully to ALWAYS call recall first, or (b) inject recalled facts into the system prompt automatically in `runner.py` before the first turn (does not depend on LLM decision). File: `prompts/system_prompt.txt:20-21`, `src/noa/orchestrator/runner.py:149-199`. | **Resolved** | MEM2 |
| MEM-M1 | Medium | Pre-compaction memory flush not implemented — OI6/CC1 spec states "before context compaction, call auto_extract to preserve user facts". Current code (`runner.py:452-486`) calls `compact_messages()` directly without extracting facts first. Facts in old messages are lost as lossy prose summary instead of being saved as searchable memory facts. Fix: before `compact_messages()`, extract facts from old messages via cheap LLM call and dispatch `memory.auto_extract` through gateway. File: `src/noa/orchestrator/runner.py:452-486`. | **Resolved** | MEM3 |
| MEM-M2 | Medium | Memory tool context description discourages proactive use — `runner.py:660` says "Use remember when the user explicitly asks" and "Use auto_extract when the user incidentally shares preferences". The word "incidentally" and "explicitly asks" make the LLM passive. Should say "ALWAYS call auto_extract when ANY personal information is shared". File: `src/noa/orchestrator/runner.py:655-664`. | Open | — |
| OI8-M1 | Medium | OI8 Smart Domain Redirect marked "Complete" in PLAN.md but only test file exists (`tests/unit/test_oi8_domain_redirect.py`) — no implementation in API endpoints or orchestrator. Currently returns 403 on domain mismatch instead of auto-creating thread in correct domain and emitting SSE redirect event. File: `src/noa/api/v1/chat.py`, `src/noa/api/v1/threads.py`. | **Resolved** | DI4 |

| RV-M1 | Medium | Idempotency cache is in-memory only — `ToolGateway._idempotency_cache` (gateway.py) and chat endpoint idempotency check (chat.py) use plain Python dicts. Lost on restart, no cross-instance support. Should move to PostgreSQL or Redis (`idempotency_keys` table with periodic cleanup). File: `src/noa/tools/gateway.py`, `src/noa/api/v1/chat.py`. | **Resolved** | DI2 |
| RV-M2 | Medium | Privacy classifier relies on static keyword matching only — `_PRIVATE_KEYWORDS` in `src/noa/privacy/classifier.py` is a small hardcoded set (journal, diary, private, personal, secret, password, confidential). Misses "medical history", "resignation letter", multi-turn private context. Should augment with local sentence-transformer classifier on Ollama or at minimum make keywords user-configurable via Settings API. File: `src/noa/privacy/classifier.py`. | **Resolved** | UX2 |
| RV-M3 | Medium | No LLM response quality evaluation test suite — all LLM-touching tests mock provider responses. No golden prompts, no fixture-based structural assertions, no offline evaluation suite. Prompt regressions or tool-calling failures in production go undetected. Should create recorded-response test suite (vcrpy cassettes or YAML fixtures) asserting structural properties (e.g. tool_calls non-empty for action prompts, privacy_mode=private for sensitive prompts). File: `tests/`. | **Resolved** | QA1 |

| W29-H1 | High | OV8 `AskUserCard` wired as dead code — `ask_user` SSE event missing from `VALID_SSE_EVENTS` in `web/src/api/sse.ts:7-14`, silently filtered at line 127. `useChatSSE.ts` has no `case "ask_user"` handler. `Chat.tsx` has no `pendingAskUser` state or `AskUserCard` render. When backend emits `ask_user` event, frontend discards it; run stays stuck in `awaiting_approval` indefinitely. Files: `web/src/api/sse.ts`, `web/src/hooks/useChatSSE.ts`, `web/src/pages/Chat.tsx`. | **Resolved** | W29-cycle2 |
| W29-H2 | High | OV10 iOS `SSEEventType` enum incomplete — missing `tool_start`, `tool_end`, `compaction`, `queued`, `ask_user`. Current enum has 12 cases vs 17 in backend `VALID_SSE_EVENT_TYPES`. iOS silently drops these events as `nil` type. Additionally `ChatRequest` struct missing `temperature: Float?` and `maxTokens: Int?` fields specified in OV10. Previous Cycle 2 review (`review_wave28-ov_cycle2.md`) falsely claimed this was fixed. File: `ios/Noa/Sources/Noa/Models/ChatModels.swift`. | **Resolved** | W29-cycle2 |
| W29-M1 | Medium | `ask_user_tool` not wired into ToolGateway — `ask_user_tool()` in `src/noa/tools/ask_user.py` uses LangGraph `interrupt()` directly but is never registered as a gateway adapter. When LLM calls `ask_user__ask_user`, `tools.py` dispatches via `_dispatch_gateway()` which returns `"Tool not registered: ask_user"`. The interrupt never fires. The frontend/iOS wiring and resume endpoint are correct; only the backend dispatch path is missing. Fix: add `AskUserAdapter` to registration.py that calls `ask_user_tool(**args)`, or handle ask_user as a pre-dispatch local-only tool. File: `src/noa/tools/registration.py`, `src/noa/tools/ask_user.py`. | **Resolved** | W29-post-audit |

| W29-AUDIT-M1 | Medium | CQ8 test references obsolete `"token"` event type — `tests/test_cq8_error_handling_sse.py:86` checks for `"token"` in `VALID_SSE_EVENT_TYPES` but canonical name is `"token_stream"` since W25b. Only real test failure (26 others are event-loop cascade). Fix: change `"token"` to `"token_stream"`. | Open | — |

**Open:** 15 | **Partially Resolved:** 0 | **Resolved:** 239 | **Total:** 254
...
### ADHOC-2026-03-17-H1: `[TASK_COMPLETE]` Marker Removed From System Prompt, Backend Contract Broken
**Severity:** High
**Status:** Open
**Description:** The system prompt rewrite (Fix 4 of the 2026-03-17 ad-hoc session) removed the `[TASK_COMPLETE]` marker instructions without updating the backend code that depends on this marker. `chat.py` lines 359-376 check `response_text` for the literal string `[TASK_COMPLETE]` to determine whether to transition a run to "completed" status. Without the marker in the system prompt, the LLM will never emit it, and every normally-completed run will remain in `"running"` status permanently. The run lifecycle is broken for standard chat interactions.

**File:** `src/noa/api/v1/chat.py`, lines 359-376 (backend contract)
**File:** `prompts/system_prompt.txt` (marker instructions removed)

**Fix:** Restore the `[TASK_COMPLETE]` section to `prompts/system_prompt.txt`. The section that must be restored:
```
## Task completion

When you have fully completed the user's task or request, end your response with the marker `[TASK_COMPLETE]`. This signals the system to close the run. Do NOT include this marker if:
- You asked a clarifying question and are waiting for input.
- You are mid-task and expect follow-up steps.
- An approval is pending.

Only include it when the task is genuinely done and no further action is needed.
```
Alternative (larger change): redesign run completion detection in `chat.py` to not rely on a prompt-injected marker.

### UI-H6: UI Does Not Adapt to Privacy Domain Switch
**Severity:** High
**Status:** Open
**Description:** The privacy mode toggle (PrivacyToggle.tsx) saves `default_privacy_mode` to user settings and the chat endpoint correctly receives it. However, no UI view reacts to the current domain. The user experience is identical in private and external mode across every page:

| View | Current behavior | Expected behavior |
|------|-----------------|-------------------|
| **Settings (Provider/Model)** | All 4 providers shown (OpenAI, Anthropic, Google AI, Ollama) | Private → only Ollama/local; External → all providers |
| **Threads sidebar** | All threads shown | Only threads matching current domain |
| **Runs page** | All runs shown | Only runs from current domain |
| **RunDetail page** | Any run accessible regardless of domain | Block or hide cross-domain run detail |
| **Cost dashboard** | Aggregates all costs | Separate private vs external costs |
| **Tools page** | Shows domain badge but no filtering | Hide tools from opposite domain |
| **Memory page** | Shows all memories | Only memories from current domain (private memory vs external memory are separate stores) |
| **Approvals page** | Shows all approvals | Only approvals from current domain (Approval model has `domain` column) |
| **Queue page** | Shows all queued tasks | Only queued tasks from current domain |
| **Artifacts page** | Shows all artifacts | Only artifacts belonging to runs in current domain |

**Root cause:** The `default_privacy_mode` setting flows correctly to the backend chat pipeline but no frontend component reads it to filter its own data. The backend APIs (`/api/v1/runs`, `/api/v1/cost/*`, `/api/v1/threads`, `/api/v1/memory`, `/api/v1/approvals`, `/api/v1/queue`) also lack a `privacy_mode` query parameter for domain-scoped queries (W22-M1 resolved runs/cost filtering on the backend, but the frontend never sends the filter).

Additionally, the router node's `PrivacyClassifier` keyword heuristics (detecting "journal", "private", "secret", etc.) are dead code — since the UI always sends an explicit `privacy_mode` value, the `user_privacy_override` path in `router_node` always takes precedence and the classifier's content analysis never fires.

**Affected files (frontend — no domain filtering):**
- `web/src/components/shared/PrivacyToggle.tsx` — toggle works, saves setting
- `web/src/components/chat/ChatComposer.tsx:56` — reads `default_privacy_mode`, sends in request
- `web/src/components/settings/GeneralSettings.tsx` — provider selector not filtered
- `web/src/pages/Runs.tsx` — no domain filter param
- `web/src/pages/RunDetail.tsx` — no domain access check
- `web/src/pages/Cost.tsx` — no domain filter param
- `web/src/pages/Memory.tsx` — no domain filter param
- `web/src/pages/Tools.tsx` — shows domain badge but doesn't filter by it
- `web/src/pages/Approvals.tsx` — no domain filter param
- `web/src/pages/Queue.tsx` — no domain filter param
- `web/src/pages/Artifacts.tsx` — no domain filter param
- `src/noa/privacy/classifier.py` — keyword heuristics never reached (dead code)

**Fix:** Every view that displays domain-specific data must read `default_privacy_mode` from settings and either (a) pass it as a query param to backend endpoints, or (b) client-side filter. The provider/model selector should conditionally show only local providers when `privacy_mode === "private"`. Backend endpoints that don't yet accept a `privacy_mode` filter param need one added.
