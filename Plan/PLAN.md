...
| **W23-FIX** | W23-H1 Fix — External Worker /data Volume | **Planned** | — | — | ~15 min | — | Data-loss bug: `MemoryStore(data_dir=Path("/data/memory/external"))` writes to ephemeral container layer — all external memory facts lost on restart. Fix: add `external-data:/data` named volume to `external-worker` service in `docker-compose.yml` and `docker-compose.dev-full.yml` (mirror private-worker pattern). Add `external-data` to the top-level `volumes:` section. Resolve W23-H1 in FINDINGS.md on completion. |
| **CX1** | Quick Fixes — Checkpointer + Idempotency + Doom Loop | **Planned** | — | — | ~45 min | — | Checkpointer upsert: replace SELECT→INSERT/UPDATE race with `INSERT ... ON CONFLICT DO UPDATE` (pg_insert). Idempotency cache: move in-memory `_idempotency_cache` dict in gateway.py to `idempotency_keys` Postgres table (key, response JSONB, created_at) with background sweeper. Survives restarts. Doom loop detection: in tool_node, hash `(tool_name, frozenset(args.items()))` per call; if same signature appears ≥3 times in the last 6 tool events → raise `DoomLoopError`, emit `error` SSE event with explanation. Threshold 3 is intentional: read→edit→read is legitimate (2), but 3 identical calls is always a loop. |
| **DI1** | Task Classifier | **Planned** | — | — | ~45 min | — | Add `task_type: Literal["simple_utility","execution","research","decision_intelligence"]` to AgentState. New `nodes/classifier.py`: cheap LLM call (Haiku/GPT-4o-mini) reads user message + thread context and outputs task_type. Runs before planner. Enables: correct rubric selection in EV1, archetype selection in OI1, selective activation of heavy workflows. Gate: simple_utility bypasses planner + eval entirely. Node-level tool policy: classifier, planner, evaluator, memory_extractor nodes receive an empty or minimal tool allowlist (hardcoded in runner.py) — only the agent node receives the full scoped allowlist from TM4. This is an internal architectural constraint, not user-configurable. |
| **MC1** | Per-Node Model Configuration | **Planned** | — | — | ~45 min | — | Expose MR8's `ModelConfig` per-node defaults to user via Settings UI. New `Settings → Intelligence` section: dropdown per node type (Classifier, Planner, Agent, Evaluator) populated from available providers/models in `providerModels.ts`. Backend: `PATCH /settings` extended with `node_models: {classifier, planner, agent, evaluator}` object; stored in `settings` table. `runner.py` reads node_models from settings at run time and passes correct model to each node's LLM call. Defaults: classifier/evaluator → cheapest available model (Haiku or GPT-4o-mini); planner → Sonnet; agent → user's current default. Also exposes model config for AI-backed tools: Transcription provider (OpenAI Whisper / whisper.cpp) and Embeddings model (nomic-embed-text / OpenAI) once VM1 exists. iOS: SettingsViewModel updated with Intelligence section. Parallel with LF1 + LS1. |
| **LF1** | Langfuse Self-Hosted + Backend Instrumentation | **Planned** | — | — | ~60 min | — | Add `langfuse` + `langfuse-postgres` services to docker-compose.yml. Install `langfuse` Python SDK. Instrument OrchestratorRunner: one trace per run (run_id as trace_id), one generation span per LLM call (prompt, completion, tokens, latency), one span per tool call. Link Langfuse trace_id to existing Run record. EV1 attaches eval scores as span metadata here. |
...
| **EV1** | Evaluation Node | **Planned** | — | — | ~60 min | — | LangGraph node after final agent response, before SSE delivery. Cheap model (Haiku/GPT-4o-mini) scores output against fixed rubric: goal_alignment / completeness / grounding / confidence_honesty / actionability (0–5 each). Archetype extensions: decision tasks add option_coverage + tradeoff_clarity; research tasks add source_quality + recency. Verdict: pass / reroute / flag. Reroute injects structured gap description into AgentState and re-enters earliest fixable node (max 2 cycles). `response_evaluations` table: run_id, task_type, archetype, rubric_version, scores JSONB, overall FLOAT, verdict, reroute_target, reroute_cycle, eval_model, eval_ms, user_rating (NULL until FB1). Eval scores attached to Langfuse trace as span metadata (LF1). Depends on LF1 + DI1 + OI1. |
| **FB1** | User Feedback Loop — Response Ratings | **Planned** | — | — | ~60 min | — | Thumbs up/down on each assistant message in ChatMessages.tsx. POST /api/v1/ratings endpoint writes to `response_ratings` table and writes back to `response_evaluations.user_rating` (user_rating becomes ground truth for rubric calibration). Quality score widget on Cost dashboard (% positive over 7d/30d). Parallel with QV1. |
| **QV1** | LLM Quality Evaluation Framework | **Planned** | — | — | ~45 min | — | vcrpy cassette fixtures for 10-15 representative prompts (tool selection, privacy routing, planner archetype selection). Tests assert rubric scores from EV1 above threshold (goal_alignment ≥ 3.5, grounding ≥ 4.0 for research tasks). Catches prompt/model regressions in CI before production. Rubric thresholds are the CI gate. Parallel with FB1. Depends on EV1. |
| **EV2** | Self-Improvement Analytics | **Planned** | — | — | ~45 min | — | Completes the quality flywheel. `GET /analytics/eval-trends`: aggregate scores by dimension / model / task_type / archetype over time. Dashboard widget: rolling quality score (7d/30d). Divergence detector: when user_rating and eval overall diverge by >1.5 → flag rubric recalibration needed. Weekly worst-dimension report surfaces lowest-scoring dimension → feeds prompt iteration loop. Depends on EV1 + FB1. |
