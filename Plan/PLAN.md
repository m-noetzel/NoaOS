...
| ID | Phase | Status | Tests | Branch | Est. | Actual | Notes |
|----|-------|--------|-------|--------|------|--------|-------|
| **CX1** | Quick Fixes — Checkpointer + Idempotency | **Planned** | — | — | ~30 min | — | Checkpointer upsert: replace SELECT→INSERT/UPDATE race with `INSERT ... ON CONFLICT DO UPDATE` (pg_insert). Idempotency cache: move in-memory `_idempotency_cache` dict in gateway.py to `idempotency_keys` Postgres table (key, response JSONB, created_at) with background sweeper. Survives restarts. |
| **LF1** | Langfuse Self-Hosted + Backend Instrumentation | **Planned** | — | — | ~60 min | — | Add `langfuse` + `langfuse-postgres` services to docker-compose.yml. Install `langfuse` Python SDK. Instrument OrchestratorRunner: one trace per run (run_id as trace_id), one generation span per LLM call (prompt, completion, tokens, latency), one span per tool call. Link Langfuse trace_id to existing Run record. |
| **LF2** | Traces Page — Langfuse Iframe Embed | **Planned** | — | — | ~45 min | — | New `/traces` route in web app. Full-height iframe embedding self-hosted Langfuse UI (langfuse:3000). AppSidebar nav entry. RunDetail "View Trace" button navigates to `/traces?traceId=<run_id>` which deep-links iframe to that specific trace. No duplicate observability UI — Langfuse owns LLM-level traces, existing Runs page owns governance/approval context. |
| **FB1** | User Feedback Loop — Response Ratings | **Planned** | — | — | ~60 min | — | `response_ratings` table (message_id, rating: thumbs_up/thumbs_down, created_at). Thumbs up/down buttons on each assistant message in ChatMessages.tsx. POST /api/v1/ratings endpoint. Quality score widget on Cost dashboard (% positive over last 7d / 30d). Gives real signal for prompt iteration and model comparison. |
...
| **PC1** | Privacy Classifier — Configurable + Semantic | **Planned** | — | — | ~60 min | — | Two improvements: (1) expose `private_keywords` as a user-configurable list in Settings UI + PATCH /settings — user can add domain-specific terms ("resignation", "medical", "salary"). (2) Add semantic scoring via sentence-transformer (`nomic-embed-text` from VM1 Ollama, zero extra infra) — cosine similarity against a "private intent" embedding anchors catches "draft my resignation letter" that keywords miss. Keyword OR semantic threshold triggers private mode. |
| **QV1** | LLM Quality Evaluation Framework | **Planned** | — | — | ~45 min | — | vcrpy cassette fixtures for 10-15 representative prompts (tool selection, privacy routing, planner strategy). Tests assert structural properties of LLM output (non-empty tool_calls, correct privacy_mode, planner selects expected tools) without live API calls. Runs in CI. Catches prompt regressions before they reach production. |
...
- **SEC1**: JWT Token Revocation — token blacklist table in Postgres (token_jti + expires_at), background sweeper purges expired entries.
