...
import { MessageSquare, Brain, Wrench, ClipboardCheck, Shield, CheckCircle2, XCircle, Package, Timer } from "lucide-react";
        ...
        return (
          <div
            ...
            className={cn(
              ...
              i === events.length - 1 ? "border-primary" : "border-border/60"
            )}
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              {event.type === "tool_result" && (
                <div className="mt-0.5 text-xs text-muted-foreground flex items-center gap-2">
                  <span>{event.data.result as string}</span>
              ...
              {event.type === "result_ready" && (
                <p className="text-sm text-muted-foreground mt-1 truncate">
                  {(event.data.response_text as string)?.slice(0, 120)}…
                </p>
              )}
